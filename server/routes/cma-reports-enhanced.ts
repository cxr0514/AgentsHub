...
// Using hexToRgb from pdfService.ts instead

// Generate CMA Report
router.post('/generate-cma', upload.single('logo'), async (req, res) => {
  try {
    log('Generating enhanced CMA report', 'cma-reports');
    
    // Use our createPdfDocument helper to handle jsPDF and autoTable import properly
    ...
    // Using doc that was created earlier with createPdfDocument()
    ...
    if (options.includeMarketAnalysis !== false && marketData && marketData.length > 0) {
      ...
      const marketDataRows = recentMarketData.map(data => [
        ...
        data.newListings ? `${data.newListings}` : 'N/A',
        data.closedSales ? `${data.closedSales}` : 'N/A',
        data.inventory ? `${data.inventory}` : 'N/A'
      ]);
      
      (doc as any).autoTable({
        startY: 35,
        head: [['Period', 'Median Price', 'Days on Market', 'Avg Price/Sqft', 'New Listings', 'Closed Sales', 'Inventory']],
        body: marketDataRows,
        theme: 'grid',
        headStyles: {
          fillColor: [r, g, b],
      ...
      if (latestData && sixMonthsAgo) {
        ...
        doc.setFontSize(14);
        doc.text('6-Month Market Trends Summary', 14, (doc as any).lastAutoTable.finalY + 20);
        
        doc.setFontSize(12);
        doc.text(`• Median home prices have ${priceChange >= 0 ? 'increased' : 'decreased'} by ${Math.abs(priceChange).toFixed(1)}% in the last 6 months.`, 20, (doc as any).lastAutoTable.finalY + 30);
        
        if (latestData.daysOnMarket && sixMonthsAgo.daysOnMarket) {
          const domChange = latestData.daysOnMarket - sixMonthsAgo.daysOnMarket;
          doc.text(`• Days on market has ${domChange >= 0 ? 'increased' : 'decreased'} by ${Math.abs(domChange)} days.`, 20, (doc as any).lastAutoTable.finalY + 40);
        }
        
        if (latestData.inventory && sixMonthsAgo.inventory) {
          const inventoryChange = ((latestData.inventory - sixMonthsAgo.inventory) / sixMonthsAgo.inventory) * 100;
          doc.text(`• Available inventory has ${inventoryChange >= 0 ? 'increased' : 'decreased'} by ${Math.abs(inventoryChange).toFixed(1)}%.`, 20, (doc as any).lastAutoTable.finalY + 50);
        }
      }
    }
    ...
    log(`Error generating enhanced CMA report: ${error.message}`, 'cma-reports');
